...
export default function GenerateImage() {
  ...
  const [productDetails, setProductDetails] = React.useState<{ name: string; category: string; description: string } | null>(null)
  ...
  const removeBackgroundResponseImage = generateProductState?.removeBackgroundResponse?.image || ''
  const removeBackgroundResponseError = generateProductState?.removeBackgroundError || ''
  const removeBackgroundImageLoading = generateProductState?.removingBackground || false
  ...
  const removeBackground = async (e?: any) => {
    ...
    formData.append('product_name', selectedImage?.product_type || removeBgProductName)
    if (selectedImage?.image?.src) {
      ...
      const blob = await response.blob()
      const file = new File([blob], `${removeBgProductName || 'product'}.${imgExt}`, {
        type: blob.type
      })
      setSelectedImage('')
