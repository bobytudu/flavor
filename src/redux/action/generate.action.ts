import { bookmarkImageFailure, bookmarkImageRequest, bookmarkImageSuccess, enhanceImageFailure, enhanceImageRequest, enhanceImageSuccess, generateImageFailure, generateImageRequest, generateImageSuccess, getNoOfImageLeftFailure, getNoOfImageLeftRequest, getNoOfImageLeftSuccess, getThemeImageFailure, getThemeImageRequest, getThemeImageSuccess, historyGeneratedImagesRequest } from 'redux/reducers/generate.reducer';
...
export const getNoOfImageLeft = async ( email: string ) => {
    try {
        ...
        let url = process.env.REACT_APP_AI_API_URL + `/api/get_number_of_images_left/`
        const response = await makeAIAPIAuthorizedRequest( { url, method: 'POST', data: { email: email } } )
...
export const generateImageCampaignBasedApi = async ( formData: any ) => {
    try {
        ...
        let url = process.env.REACT_APP_AI_API_URL + '/api/generate_image_target_audience'
        const headers = {
        ...
        if ( response.status === 200 ) {
            ...
            if ( generatedImageState && generatedImageState?.length > 0 ) {
                let historyGeneratedImageState = state.generate['historyGeneratedImages']
                historyGeneratedImageState = [...generatedImageState, ...historyGeneratedImageState]
...
export const generateImageCustomStudioApi = async ( formData: any ) => {
    try {
        dispatch( generateImageRequest() )
        let url = process.env.REACT_APP_AI_API_URL + '/api/generate_image_custom_studio'
        const headers = {
            'Content-Type': 'multipart/form-data'
        }
        const response = await makeAIAPIAuthorizedRequest( { url, method: 'POST', data: formData, headers } )
...
export const generateImageVisualConceptApi = async ( formData: any ) => {
    try {
        dispatch( generateImageRequest() )
        let url = process.env.REACT_APP_AI_API_URL + '/api/generate_image_visual_concept'
        const headers = {
            'Content-Type': 'multipart/form-data'
        }
        const response = await makeAIAPIAuthorizedRequest( { url, method: 'POST', data: formData, headers } )
...
export const enhanceImageApi = async ( formData: any, arrIndex: any, imageListState: any ) => {
    try {
...
export const bookmarkImage = async ( formData: any, arrIndex: number, imageListState: string ) => {
    try {
